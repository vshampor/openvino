...
bool ov::pass::SDPAToPagedAttention::run_on_model(const std::shared_ptr<ov::Model>& model) {
    ...
    if (m_allow_score_aggregation) {
        auto block_indices = setName(std::make_shared<v0::Parameter>(element::i32, PartialShape{-1}), "score_aggregation_window");
        optional_model_wide_params["score_aggregation_window"] = block_indices;
    }
