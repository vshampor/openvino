...
void rotate_kv_cache(PlainTensor& key_cache,
    ...
    for (size_t i = 0; i < num_rotated_blocks; i++) {
        ...
        int32_t* rotation_deltas_block_data = rotation_deltas.ptr<int32_t>() + i * block_chunk_size;
...
struct MHAHelper {
    ...
    void init(size_t H, size_t S, size_t SV, size_t Hk, size_t h_each_group_len, size_t block_size, size_t sliding_window,
              float d_scale, size_t kv_len, bool init_alibi_lookup, bool init_rotation_coefficient_scratch) {
        // query shape: [B, H, L, S]
        ...
        if (init_rotation_coefficient_scratch) {
            _qk_scratch_a.resize<DATA_TYPE>({_block_size, S});
        }
    }
    ...
    void exec_loop_bhl(const PlainTensor& query,
                    ...
                    for (size_t pq = 0; pq < q_len; pq++) {
                        for (size_t h = hk * _h_each_group_len; h < (hk + 1) * _h_each_group_len; h++) {
                            (*_gemv)(query.ptr<DATA_TYPE>(b, h, pq),
                                     key_cache.ptr<KVCACHE_TYPE>(block_number, hk),
        ...
        parallel_for3d_dynamic(B, kv_len_in_blocks, _Hk, [&](size_t b, size_t pv_in_blocks, size_t hk) {
            ...
            if (pv < context_len) {
                ...
                auto* v = value_cache.ptr<KVCACHE_TYPE>(block_number, hk);
                for (size_t pq = 0; pq < q_len; pq++) {
                    for (size_t h = hk * _h_each_group_len; h < (hk + 1) * _h_each_group_len; h++) {
...
struct AttentionExecutor : public PagedAttentionExecutor {
    ...
    void init(const std::vector<MemoryPtr>& inputs,
        ...
        if (rotated_block_indices) {
            ...
            rotation_trig_lut.assert_dims({0, S});
